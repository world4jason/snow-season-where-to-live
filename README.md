...
GitHub Pages (UI)
  ...
  └─ concise ski-resort profile (map / ticket / official link / size / tags / access)
          ↓
Cloudflare Worker
  ├─ GET  /api/latest
  ├─ POST /api/search
...
- **Special-season exclusions:** `config/excluded-resorts.json` removes destinations that are poor fits for a normal continuous winter lodging search. Senjojiki (spring-only) and Okutadami Maruyama (split early-winter + spring operation with a long midwinter closure) are currently excluded from the main UI.

The raw ranking union currently contains **48 source lodging bases**; after the two special-season exclusions, the main destination browser exposes **46 standard-winter lodging bases**.
...
- **雪場簡介** — deliberately concise ski context for the selected lodging base:
  - map
...
Profile metadata lives in `config/resort-profiles.json`.
...
- Daily automatic results seed the same cache for the five core monitored areas.
